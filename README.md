...
You can define the feature categories in the confuration file (config/server.json). Note, category id 0 is reserved for the simple, single value feature toggle category (however, you can still specify it in the config to set the name and description).
...
By default Hobknob ships with authentication disabled. This is configurable by changing the config/server.json config file.
...
The configuration for the session is also stored in the config/server.json file using the following:
...
```
## Configuring Logging
By default, express has been configured to use a dev logger to stdout. You can configure to use different logging middleware by supplying configuration in config/server.json file.
